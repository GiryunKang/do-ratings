...
export default function ImageAttribution({ attribution }: ImageAttributionProps) {
  return (
    <div className="flex items-center gap-1 text-[10px] text-gray-400 mt-1">
      ...
      {attribution.url ? (
        <a href={attribution.url} target="_blank" rel="noopener noreferrer" className="hover:text-gray-600 hover:underline">
      ...
      {attribution.license && (
        <span className="text-gray-300">({attribution.license})</span>
      )}
    </div>
  )
}
